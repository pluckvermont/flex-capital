...
function wpiw_widget_cr() {
	register_widget( 'cr_instagram_widget' );
}
add_action( 'widgets_init', 'wpiw_widget_cr' );

class cr_instagram_widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'null-instagram-feed',
			__( 'Creativo Instagram Widget', 'wp-instagram-widget' ),
			array(
				...
				foreach ( $media_array as $item ) {
					...
					if ( locate_template( $template_part ) != '' ) {
						...
						echo '<li class="'. esc_attr( $liclass ) .'"><a href="'. esc_url( $item['link'] ) .'" target="'. esc_attr( $target ) .'"  class="'. esc_attr( $aclass ) .'"><img src="'. esc_url( $item[$size] ) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"  class="'. esc_attr( $imgclass ) .'"/></a></li>';
							...
							echo '<li><a href="'.esc_url( $item['original'] ).'" rel="prettyPhoto[\'instagram\']" class="'. esc_attr( $aclass ) .'"><img src="'.esc_url( $item[$size] ).'"  class="'. esc_attr( $imgclass ) .'"></a></li>';
						}
					}
				}
				?></ul><?php
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>"><?php esc_html_e( 'Open links in', 'wp-instagram-widget' ); ?>:</label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'target' ) ); ?>" class="widefat">
				...
				<option value="prettyphoto" <?php selected( 'prettyphoto', $target ) ?>><?php esc_html_e( 'Pretty Photo - same page', 'wp-instagram-widget' ); ?></option>
			</select>
		</p>
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['target'] = ( ( $new_instance['target'] == '_self' || $new_instance['target'] == '_blank' || $new_instance['target'] == 'prettyphoto' ) ? $new_instance['target'] : '_self' );
